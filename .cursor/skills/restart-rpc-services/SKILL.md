...
description: >-
  ...
  仅针对包含 RPC/HTTP API 调用的程序（微信小程序等）；纯静态 Web/Pyodide 或纯 CLI 不在此列。
...
- [ ] 已重启本地含 RPC 的服务：python3 apps/miniprogram_api.py（默认端口 8765）
- [ ] 已确认服务打印「小程序仿真 API 运行于 http://127.0.0.1:8765」或等价就绪日志
...
```text
小程序仿真 API 运行于 http://127.0.0.1:8765
```
