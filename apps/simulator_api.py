"""移动端统一仿真 HTTP API（微信小程序 + iOS App 共用）。"""
...
# 支持从项目根目录直接运行：python3 apps/simulator_api.py
...
def build_data_payload() -> dict[str, Any]:
    """返回航母/战斗机数据库 JSON（与 build_miniprogram / iOS data.json 结构一致）。"""
...
def handle_request(method: str, path: str, body: bytes | None) -> tuple[int, dict[str, str], bytes]:
    ...
    if method == 'POST' and route == '/api/simulate':
        ...
        try:
            ...
            print(f'[simulator_api] 仿真未捕获异常: {exc}')
...
class _ApiHandler(BaseHTTPRequestHandler):
    ...
    def log_message(self, format: str, *args: Any) -> None:
        print(f'[simulator_api] {self.address_string()} - {format % args}')
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='航母起飞仿真 — 统一 HTTP API（小程序 / iOS）')
