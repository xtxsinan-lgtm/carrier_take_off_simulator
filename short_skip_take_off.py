...
# 预计算候选值（用于搜索）
THETA_NOZZLE_TAKEOFF_LIST_A = range(10, 41, 5)      # 起飞时主喷管偏转角度候选列表，单位 度。
FLAT_LENGTH_LIST_A = range(50, 301, 10)             # 平直段从 40m 到 200m，步长 20m
V_TRANS_LIST_A = range(10, 51, 10)                   # 喷口开始偏转时地速 15m/s 到 75m/s，步长 10m/s
THETA_NOZZLE_TAKEOFF_LIST_B = range(0, 31, 10)      # 起飞时主喷管偏转角度候选列表，单位 度。
FLAT_LENGTH_LIST_B = range(100, 501, 10)             # 平直段从 80m 到 300m，步长 20m

THETA_DEG_PITCH_LIST = range(20, 21, 1)   # 飞机俯仰角候选列表，单位 度。
...
def simulate_optimal(flat_length, V_trans, theta_deg_takeoff, strategy, theta_pitch_deg, dt=0.02):
    """
    ...
    theta_pitch_air = np.radians(theta_pitch_deg)     # 离甲板后固定俯仰角（弧度）
    ...
    while x < flat_length and t < 30:               # 当飞机未跑出平直段且时间未超 30 秒时循环
        ...
        if in_trans:
            ...
            theta_nozzle = trans_start + (theta_final -trans_start) * ratio      # 当前喷管角 = 最终角 × 过渡比例（线性插值）
        ...
        V = max(V + a * dt, 0)                      # 更新地速
        x += V * dt                                 # 更新水平位置
        # if t < fix_nozzle_time: 
        #     print(x, t)
        t += dt                                     # 时间推进
        if t < fix_nozzle_time + dt and t > fix_nozzle_time - dt:
            buffer_length = x
    # ==================== 阶段2: 滑跃甲板滑跑 ====================
    ...
    if s < SKI_JUMP_LENGTH * 0.99:
        # 如果滑跃段未走完，判定起飞失败
        return False, x, V, t, 0, buffer_length
    ...
    if Vy < 0:
        return False, x_deck, V_deck, t_deck, min_Vy, buffer_length

    # 固定控制角度（不再实时搜索）
    theta_p = theta_pitch_air                       # 固定俯仰角
    theta_n = theta_final                      # 固定喷管偏转角
    ...
    while t_air < 3.0:                              # 最多仿真离甲板后 3 秒
        ...
        thrust_ang = theta_p + theta_n                # 主喷管推力向量相对水平面的绝对角度
    ...
    return True, x_deck, V_deck, t_deck, min_Vy, buffer_length
        ...
        for theta_deg in THETA_NOZZLE_TAKEOFF_LIST_A:       # 起飞喷管最终偏转角
            for Vt in V_TRANS_LIST_A:                       # 转换地速
                for theta_p_deg in THETA_DEG_PITCH_LIST:   # 离甲板后固定俯仰角（粗搜 0°~20°，步长5°）
                    s, xd, vd, t_deck, mvy, buffer_length = simulate_optimal(flat, Vt, theta_deg, 'A', theta_p_deg)
                    if s:
                        total = flat + SKI_JUMP_LENGTH * ski_cos
                        results_A.append((total, flat, Vt, theta_deg, theta_p_deg, vd, t_deck, mvy))
                        if best_A is None or total < best_A[0]:
                            return (total, flat, Vt, theta_deg, theta_p_deg, vd, t_deck, mvy, buffer_length)
    return None
...
# 细化搜索
def fine_tune_a():
    for flat in range(max(int(best_A[1]) - 20, 0), int(best_A[1]), 1):
        for Vt in range(best_A[2] - 10, best_A[2] + 11, 1):
            for theta_deg in range(max(best_A[3] - 5, 1), best_A[3] + 6, 1):
                if theta_deg > 90: continue
                for theta_p_deg in range(max(best_A[4], 0), min(best_A[4] + 3, 21), 1):
                    s, xd, vd, t_deck, mvy, buffer_length = simulate_optimal(flat, Vt, theta_deg, 'A', theta_p_deg)
                    if s:
                        total = flat + SKI_JUMP_LENGTH * ski_cos
                        if total < best_A[0]:
...
if best_A:
    ...
    print(f"        平直段={best_A[1]:.0f}m 转换地速={best_A[2]}m/s 喷管最终角度={best_A[3]}°")
    print(f"★ 最优: 总距={best_A[0]:.1f}m ({best_A[0]*3.28084:.0f}ft)")
    print(f"        平直段={best_A[1]:.0f}m 转换地速={best_A[2]}m/s 喷管最终角度={best_A[3]}°")
    print(f"        离甲板后固定俯仰角={best_A[4]}° 离甲板速度={best_A[5]:.1f}m/s 离甲板时间={best_A[6]:.1f}s 缓冲长度={best_A[7]}")
    print("细化搜索")
    best_A = fine_tune_a()
    print(f"★ 细化最优: 总距={best_A[0]:.1f}m ({best_A[0]*3.28084:.0f}ft)")
    print(f"        平直段={best_A[1]:.0f}m 转换地速={best_A[2]}m/s 喷管最终角度={best_A[3]}°")
    print(f"        离甲板后固定俯仰角={best_A[4]}° 离甲板速度={best_A[5]:.1f}m/s 离甲板时间={best_A[6]:.1f}s 缓冲长度={best_A[7]}")
    ...
    for flat in FLAT_LENGTH_LIST_B:
        for theta_deg in THETA_NOZZLE_TAKEOFF_LIST_B:
            for theta_p_deg in THETA_DEG_PITCH_LIST:
                s, xd, vd, t_deck, mvy, buffer_length = simulate_optimal(flat, 0, theta_deg, 'B', theta_p_deg)
                if s:
                    total = flat + SKI_JUMP_LENGTH * ski_cos
                    if best_B is None or total < best_B[0]:
                        return (total, flat, theta_deg, theta_p_deg, vd, t_deck, mvy)
    return None
...
def fine_tune_b():
    for flat in range(max(int(best_B[1]) - 20, 0), int(best_B[1]), 1):
        for theta_deg in range(max(best_B[2] - 10, 0), best_B[2] + 11, 1):
            if theta_deg > 90: continue
            for theta_p_deg in range(max(best_B[3], 0), min(best_B[3] + 3, 21), 1):
                s, xd, vd, t_deck, mvy, buffer_length = simulate_optimal(flat, 0, theta_deg, 'B', theta_p_deg)
                if s:
                    total = flat + SKI_JUMP_LENGTH * ski_cos
                    if total < best_B[0]:
...
if best_B:
    ...
    print(f"        平直段={best_B[1]:.0f}m 固定喷管角度={best_B[2]}°")
    print(f"        离甲板后固定俯仰角={best_B[3]}° 离甲板速度={best_B[4]:.1f}m/s 离甲板时间={best_A[5]:.1f}s")
    print("细化搜索")
    best_B = fine_tune_b()
    print(f"★ 细化最优: 总距={best_B[0]:.1f}m ({best_B[0]*3.28084:.0f}ft)")
    print(f"        平直段={best_B[1]:.0f}m 固定喷管角度={best_B[2]}°")
    print(f"        离甲板后固定俯仰角={best_B[3]}° 离甲板速度={best_B[4]:.1f}m/s 离甲板时间={best_A[5]:.1f}s")
...
if best_A and best_B:
    ...
    print(f"       平直段={best_A[1]:.0f}m, 开始转换地速={best_A[2]}m/s, 最终喷管角度={best_A[3]}°")
    print(f"       离甲板后固定俯仰角={best_A[4]}°")
    print(f"策略B: {best_B[0]:.1f} m ({best_B[0]*3.28084:.0f} ft)")
    print(f"       平直段={best_B[1]:.0f}m, 固定角度={best_B[2]}°")
    print(f"       离甲板后固定俯仰角={best_B[3]}°")
