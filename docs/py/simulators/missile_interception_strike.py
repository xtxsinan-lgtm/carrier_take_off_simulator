...
# 支持直接运行：python3 simulators/missile_interception_strike.py
_ROOT = Path(__file__).resolve().parent.parent
if str(_ROOT) not in sys.path:
    sys.path.insert(0, str(_ROOT))

from utils.missile_interception_config import estimate_defaults, physics_config, simulation_config, ui_config
from utils.missile_interception_monte_carlo import optimize_plan
from utils.missile_interception_radar import (
    binding_limit_label,
    estimate_engagement_distance,
    estimate_pk,
)
from utils.missile_interception_windows import compute_windows
